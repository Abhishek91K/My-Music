...
let songs = [];
let currFolder = "";
const baseURL = "https://lbvkxbtwnitlhangidqd.supabase.co/storage/v1/object/public/songs";

function convertToMinSec(seconds) {
    const mins = Math.floor(seconds / 60);
    const secs = (seconds % 60).toFixed(0); // Keep 2 decimal points

    // Pad seconds if less than 10 (including decimals)
...
async function main() {
    ...
    next.addEventListener("click", () => {
        let x = currentSong.src.split("/").slice(-1)[0].split(".mp3").slice(0)[0].replaceAll("%20", " ");
        let index = songs.indexOf(x);
